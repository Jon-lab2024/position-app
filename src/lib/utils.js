...
/**
* Get user location and check if it's being computed from GNSS
* @returns A promise that resolves with the user's coordinates, GNSS status, and a message
*/
  ...
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        console.log('Received position:', coords);

        // Increased the accuracy threshold to 50 meters to make it more suitable for indoor environments
        const isGNSS = coords.accuracy <= 50;
        console.log('Is GNSS?', isGNSS, 'Accuracy:', coords.accuracy);

        resolve({
          coords,
          isGNSS,
          message: isGNSS ? "GNSS is available" : "GNSS is not available",
        });
      },
      (error) => {
